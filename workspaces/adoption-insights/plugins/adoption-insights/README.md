...
The plugin supports both the **legacy** Backstage frontend and the **New Frontend System (NFS)**. NFS is the primary package entry point. OFS (legacy) exports are available only at `./legacy`. Raw translation resources (for OFS/dynamic plugins) remain at `./alpha`; NFS apps use `adoptionInsightsTranslationsModule` from the package root.
